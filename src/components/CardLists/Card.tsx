...
const Card = ({ list, category }: ListProps) => {
  ...
  return (
    <Link to={`/${category}/${list?.id}`} className='cursor-pointer'>
      <div
        key={uuidv4()}
        className='z-20 group relative cursor-pointer w-44 lg:w-195px md:w-60 h-72 hover:bg-black bg-white overflow-hidden rounded-3xl flex-1 '
      >
        <img
          className='z-20 w-full h-full hover:brightness-50 transition duration-300 ease-in-out '
          src={!bg.includes('null') ? bg : noImage}
          alt='imovies '
        />
        <Button className=' z-0 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-12 rounded-full   transform scale-0 group-hover:scale-100 transition duration-300 ease-in-out' />
